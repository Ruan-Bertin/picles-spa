...
export function PetDetails() {
    ...
    const {data: petData, isLoading, isError: petIsError} = useQuery({queryKey: ['get-pet-by-id', id],
        queryFn: async () => { return await getPetById(id ?? '')},
    })
                        ...
                        !petIsError && (
                            <>    
                                ...
                                <p>{petData?.bio}</p>
                                {
                                    !shelterIsError && 
                                    <a 
                                    href={`https://wa.me/${shelterData?.shelterWhatsApp}?text='Olá, gostaria de falar sobre 
                                    ${petData?.name}'`} target="_blank">
